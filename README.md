...
Set `noindex: true` to exclude a post from search engine indexing — it's dropped from `sitemap.xml` and gets its own `Disallow` line in `robots.txt`, but is otherwise a normal published post (still shown on index pages, category pages, the feed, the archive, and post navigation). Unlike `draft`, it doesn't hide the post anywhere except search indexing. If `noindex` is absent or `false`, the post is indexed normally.
