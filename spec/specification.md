...
- `date` — a post's `date` frontmatter value; `null` for every page type that isn't a single dated post (index/category/notes pages, the archive, special pages without a date).
...
- Index pages use `index_title` from config (falling back to `site_name` if unset); category pages use the category's display name; the notes page uses "Short notes"; the archive uses "Archive". Pages 2 and beyond of a paginated set append `" (page N)"`.
