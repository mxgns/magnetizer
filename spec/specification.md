...
- `<meta name="description">` appears only on index pages, using `index_meta_description` from config, if set. `index.html` (page 1) uses it verbatim; `index-2.html` and beyond append ` - Page N`, the same page-number suffix as `<title>`, so paginated index pages don't all carry an identical description.
