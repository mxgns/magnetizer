...
class TestGitFailure:
    ...
    def _failing_mock_no_staged(self, fail_cmd_prefix):
        import subprocess as sp
        def side_effect(cmd, **kwargs):
            if cmd == ["git", "diff", "--cached", "--quiet"]:
                m = MagicMock()
    ...
    def test_git_push_failure_raises_runtime_error_when_ahead_with_no_staged(self, tmp_path):
        with patch("magnetizer.publisher.subprocess.run",
                   side_effect=self._failing_mock_no_staged(["git", "push"])):
            with pytest.raises(RuntimeError):
                publish(tmp_path, "2026-05-24 14:32:00")

    def test_git_push_failure_message_includes_stderr_when_ahead_with_no_staged(self, tmp_path):
        with patch("magnetizer.publisher.subprocess.run",
                   side_effect=self._failing_mock_no_staged(["git", "push"])):
            with pytest.raises(RuntimeError, match="push failed"):
                publish(tmp_path, "2026-05-24 14:32:00")
