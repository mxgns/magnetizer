...
def make_project(tmp_path, missing=None):
    ...
    if missing != "templates/index.html" and "templates" not in (missing or ""):
        (tmp_path / "templates" / "index.html").write_text("<html>MAGNETIZER_METADATA MAGNETIZER_CONTENT</html>")
    return tmp_path
...
class TestValidateContentValid:
    ...
    def test_passes_with_svg_extension(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.svg": MINIMAL_IMG,
        })
        validate_content(content)
...
# ---------------------------------------------------------------------------
# validate_content — image numbering gaps
# ---------------------------------------------------------------------------

class TestValidateContentImageNumberingGaps:

    def test_passes_with_sequential_numbering(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.jpg": MINIMAL_IMG,
            "1-image-02.jpg": MINIMAL_IMG,
            "1-image-03.jpg": MINIMAL_IMG,
        })
        validate_content(content)

    def test_passes_with_single_image_at_01(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.jpg": MINIMAL_IMG,
        })
    ...
    def test_fails_when_gap_in_middle(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.jpg": MINIMAL_IMG,
            "1-image-03.jpg": MINIMAL_IMG,  # missing 02
        })
        with pytest.raises(SystemExit):
            validate_content(content)

    def test_fails_when_numbering_does_not_start_at_01(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-02.jpg": MINIMAL_IMG,
            "1-image-03.jpg": MINIMAL_IMG,
    ...
    def test_error_message_names_post_and_missing_number(self, tmp_path, capsys):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.jpg": MINIMAL_IMG,
            "1-image-03.jpg": MINIMAL_IMG,
    ...
    def test_gap_in_one_post_does_not_affect_another(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "1-image-01.jpg": MINIMAL_IMG,
            "1-image-03.jpg": MINIMAL_IMG,  # gap
            "2.md": MINIMAL_MD,
            "2-image-01.jpg": MINIMAL_IMG,
            "2-image-02.jpg": MINIMAL_IMG,  # sequential
        })
        with pytest.raises(SystemExit):
            validate_content(content)
...
# ---------------------------------------------------------------------------
# validate_content — special pages
# ---------------------------------------------------------------------------

class TestValidateContentSpecialPages:

    def test_passes_with_configured_special_page_md(self, tmp_path):
    ...
    def test_passes_with_configured_special_page_md_and_image(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "about.md": MINIMAL_MD,
    ...
    def test_passes_with_configured_special_page_svg_image(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "about.md": MINIMAL_MD,
            "about-image-01.svg": MINIMAL_IMG,
        })
        validate_content(content, config={"special_pages": ["about"]})  # should not raise

    def test_passes_with_multiple_configured_special_pages(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "about.md": MINIMAL_MD,
            "cookies.md": MINIMAL_MD,
            "now.md": MINIMAL_MD,
        })
        validate_content(content, config={"special_pages": ["about", "cookies", "now"]})  # should not raise

    def test_special_page_md_alone_without_posts_fails(self, tmp_path):
        content = make_content(tmp_path, {"about.md": MINIMAL_MD})
        with pytest.raises(SystemExit):
            validate_content(content, config={"special_pages": ["about"]})

    def test_fails_when_configured_special_page_md_missing(self, tmp_path):
        content = make_content(tmp_path, {"1.md": MINIMAL_MD})
        with pytest.raises(SystemExit):
            validate_content(content, config={"special_pages": ["about"]})

    def test_error_message_names_missing_special_page(self, tmp_path, capsys):
        content = make_content(tmp_path, {"1.md": MINIMAL_MD})
        with pytest.raises(SystemExit):
            validate_content(content, config={"special_pages": ["about"]})
        assert "about.md" in capsys.readouterr().err

    def test_special_page_md_without_config_entry_is_unrecognised(self, tmp_path):
        content = make_content(tmp_path, {"1.md": MINIMAL_MD, "about.md": MINIMAL_MD})
        with pytest.raises(SystemExit):
            validate_content(content)  # no config passed, so "about" isn't a recognised special page

    def test_special_page_image_without_matching_md_fails(self, tmp_path):
        content = make_content(tmp_path, {
            "1.md": MINIMAL_MD,
            "about-image-01.jpg": MINIMAL_IMG,
        })
...
class TestValidateConfig:
    ...
    def test_fails_when_category_slug_matches_configured_special_page(self):
        with pytest.raises(SystemExit):
            validate_config({
                "site_url": "https://example.github.io",
                "special_pages": ["about"],
                "categories": {"about": "Whatever"},
            })

    def test_passes_with_special_page_name_as_category_slug_when_not_configured(self):
        validate_config({
            "site_url": "https://example.github.io",
            "categories": {"about": "Whatever"},
        })  # "about" isn't reserved unless it's actually configured as a special page
